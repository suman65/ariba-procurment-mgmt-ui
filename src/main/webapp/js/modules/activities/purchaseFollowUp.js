var PURCHASE_FOLLOWUP = {
	init	: function()
	{
		this.definePurchaseFallowUp();
	}
	,definePurchaseFallowUp : function()
	{
		Ext.define('PurchaseFollowUp', 
		{
	...
	,getPurchaseFollowUpStore: function(extraParams)
	{
		var proxy = {
				type		: 'ajax'
				,url		: ATP.Urls.gridData
				,extraParams: {actionType : 'purchaseFollowupService'}
				,reader		: 'jsonreader'
			}
	...
	,getPurchaseFollowUpGrid : function()
	{
		var me			=	this;
		var purchaseFollowUpGrid	=	{
			xtype		: 'customgrid'
			,itemId		: 'purchaseFollowUpGrid'
			,store		: me.getPurchaseFollowUpStore()
			,flex		: 1
			,columns		: 
			[
		...
		return purchaseFollowUpGrid;
	}
	,getPurchaseFollowUpPanel: function()
	{
		var me	=	this,
			panel = {
				xtype		: 'panel'
				...
				[
					...
					,{
						...
						,mask: function (msg, msgCls, elHeight)
						{
							...
							target.mask('Select a record from left.', 'purchaseFollowUpMaskCls', elHeight);
							this.setMasked(true);
						}
					}
				]
	...
	,getTemplate: function()
	{
		return new Ext.XTemplate(
			'<table class="purchaseFollowUp">'
			,'<tbody>'
				,'<tr style="background-color: #a1968a; font-size: 15px;">'
					,'<th colSpan="5" style="">Purchase FollowUp Summary</th>'
				,'</tr>'
				,'<tr class="highlight">'
					,'<th>Data</th>'
				...
				,'<tr>'
					...
					,'<td style="text-align: right; padding-right: 6px;">{plan.hybridCropArea}</td>'
					,'<td style="text-align: right; padding-right: 6px; background-color: yellowgreen;">{actual.hybridCropArea}</td>'
				...
				,'<tr class="highlight">'
					...
					,'<td style="text-align: right; padding-right: 6px;">{[values.plan.pioneerHybrid1Area + values.plan.pioneerHybrid2Area + values.plan.competitorHybrid1Area + values.plan.competitorHybrid2Area + values.plan.competitorHybrid3Area + values.plan.competitorHybrid4Area + values.plan.competitorHybrid5Area]}</td>'
					,'<td style="text-align: right; padding-right: 6px; background-color: yellowgreen;">{[values.actual.pioneerHybrid1Area + values.actual.pioneerHybrid2Area + values.actual.competitorHybrid1Area + values.actual.competitorHybrid2Area + values.actual.competitorHybrid3Area + values.actual.competitorHybrid4Area + values.actual.competitorHybrid5Area]}</td>'
			...
			,{
				...
				,getChangePer: function()
				{
					...
					var changePercentage = value2 != 0 ? ((value1 - value2) / value2 * 100).toFixed(2) : null;
	...
	,updateDetails: function(container, record)
	{
		...
		Ext.Ajax.request(
		{
			url 		: ATP.Urls.getPurchaseFollowUpSummary
			,method		: 'GET'
